...
<jsp:useBean id="requestutil" class="weaver.workflow.request.todo.RequestUtil" scope="page" />*/
	...
	System.out.println("wfTabFrame==>upcoming:" + upcoming);
...
function getNumCount(){
    $.ajax({
  		...
  		data: { workflowid: "<%=workflowid%>", typeid: "<%=typeid%>",offical:"<%=offical%>",officalType:"<%=officalType%>", upcoming: "<%=upcoming%>"},
...
function attachUrl(){
	...
	$("a[target='tabcontentframe']").each(function(){
		var	url = "/workflow/search/WFSearchTemp.jsp?upcoming=<%=upcoming%>&"+requestParameters;
...
function reloadLeftNum(){
	...
	var url = "/workflow/request/RequestViewAjaxCount.jsp?<%=request.getQueryString() %>&upcoming=<%=upcoming%>";
	try {
		url = parent.getajaxcounturl() + "&<%=request.getQueryString() %>&upcoming=<%=upcoming%>";
	} catch (e) {}
