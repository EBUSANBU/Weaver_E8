...
*/
    ...
    System.out.println("wfTabFrameCount==>upcoming:" + upcoming);
    ...
    if (typeid.equals("") && workflowid.equals("") && !offical.equals("1")) {
        ...
        if (upcoming.equals("0")){  //待办不含抄送
            for (int a = 0; a < countArr.size(); a++) {
                ...
                if (countArr.get(a).equals("flowOut")) {
                    ...
                    sqlsb.append("	  where (((isremark='0' and (takisremark is null or takisremark=0 )) and (isprocessed is null or (isprocessed <> '2' and isprocessed <> '3'))) or isremark in('1','5','7')) ");
                } else if (countArr.get(a).equals("flowSup") || countArr.get(a).equals("flowResponse")) {
                    sqlsb.append("    where (((isremark='0' and (takisremark is null or takisremark=0 )) or isremark in('1','5','7'))  or (isremark = '0' and isprocessed is null))");
                } else {
                    sqlsb.append("	  where ((isremark='0' and (takisremark is null or takisremark=0 )) or isremark in('1','5','7')) ");
                }
                ...
                if (countArr.get(a).equals("flowNew"))
                    sql += " and a.viewtype = '0' and a.isremark != '5' and a.isremark != '8' and a.isremark != '9' and a.isprocessed is null ";
                else if (countArr.get(a).equals("flowResponse"))
                    sql += " and a.viewtype = '-1'  and a.isremark != '8' and a.isremark != '9' ";
                    //else if(countArr.get(a).equals("flowOut"))
                    //sql += " and a.isremark = '5' ";
                else if (countArr.get(a).equals("flowSup"))
                    sql += " and a.requestid in (select requestid from workflow_requestlog where logtype='s')  and a.isremark != '8' and a.isremark != '9' ";
                else
                    sql += " and a.isremark != '8' and a.isremark != '9' ";
            ...
            for (int a = 0; a < countArr.size(); a++) {
                if (!countArr.get(a).equals("flowAll")) {
                    continue;
                }
                StringBuffer sqlsb = new StringBuffer();
                if ("1".equals(belongtoshow)) {
                ...
                sqlsb.append("	  where (isremark in('8','9')) ");
        ...
        } else {  //如果url不含upcoming 归于此，原本代码，防止其他文件调用失败
            for (int a = 0; a < countArr.size(); a++) {
                StringBuffer sqlsb = new StringBuffer();
                if ("1".equals(belongtoshow)) {
                    sqlsb.append("select count(a.requestid) wfCount ");
        ...
        //加上异构系统数据的数量
     /*   if (requestutil.getOfsSetting().getIsuse() == 1 && !upcoming.equals("1")) {
            //全部待办
