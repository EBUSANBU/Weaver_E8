...
<form name=frmmain method=post action="RequestView.jsp?offical=<%=offical %>&officalType=<%=officalType %>">
    ...
    /* edited by wdl 2006-06-14 left menu advanced menu */
        if(fromAdvancedMenu!=1){
        	RCMenuWidth = 160;
	        //RCMenu += "{"+SystemEnv.getHtmlLabelName(16347,user.getLanguage())+",/workflow/search/WFSearchTemp.jsp?method=all&complete=0&upcoming=0&viewType=1,_self}" ;  //并没有走
    ...
    %>
		...
		<script type="text/javascript">
			...
			function onloadtree()
			{
				var ajax=ajaxinit();
       			...
       			ajax.send("loadtree=true&upcoming=0");
			...
			function onloadtreeCount(obj,menus,options,level,customparams){
				jQuery(obj).leftNumMenu("/workflow/request/RequestViewAjaxCount.jsp?<%=request.getQueryString() %>&upcoming=0","update");
			}
			
			function getajaxcounturl() {
				return "/workflow/request/RequestViewAjaxCount.jsp?resourceid=<%=Util.null2String(request.getParameter("resourceid")) %>&upcoming=0";
			}
			...
			function leftMenuClickFn(attr,level,numberType){
				...
				url+="&upcoming=0&viewScope=doing&numberType="+numberType+"&flowAll="+flowAll+"&flowNew="+flowNew+"&flowResponse="+flowResponse+"&flowOut="+flowOut+"&flowSup="+flowSup;
			...
			function WfSearchAll(){
				...
				/*清除右侧查询条件并重新查询*/
				document.getElementById("myFrame").src ="/workflow/search/wfTabFrame.jsp?offical=<%=offical %>&officalType=<%=officalType %>&method=all&upcoming=0&viewScope=doing&complete=0&wftypes=<%=wftypes %>";
			}
		</script>
		...
		<table cellspacing="0" cellpadding="0" class="flowsTable"  >
			<tr>
				...
				<td rowspan="2">
						<iframe id="myFrame" name="myFrame"  src="/workflow/search/wfTabFrame.jsp?offical=<%=offical %>&officalType=<%=officalType %>&method=all&upcoming=0&viewScope=doing&complete=0&wftypes=<%=wftypes %>"
