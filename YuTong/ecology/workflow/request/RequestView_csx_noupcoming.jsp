...
<%
    ...
    //SQL = "select workflowtype, workflowid from workflow_currentoperator where (isremark='0' or isremark='1' or isremark='5' or isremark='8' or isremark='9' or isremark='7')  and islasttimes=1 and userid=" +  resourceid  + " and usertype= " + usertype +"  and exists (select 1 from workflow_requestbase c where c.workflowid=workflow_currentoperator.workflowid and c.requestid=workflow_currentoperator.requestid)";
   	if(loadtree.equals("true")){
	    ...
	    //System.out.println("requestView: "+sqlsb.toString());
	    while(RecordSet.next()){
...
<form name=frmmain method=post action="RequestView.jsp?offical=<%=offical %>&officalType=<%=officalType %>">
    ...
    /* edited by wdl 2006-06-14 left menu advanced menu */
        if(fromAdvancedMenu!=1){
        	RCMenuWidth = 160;
	        //RCMenu += "{"+SystemEnv.getHtmlLabelName(16347,user.getLanguage())+",/workflow/search/WFSearchTemp.jsp?method=all&complete=0&upcoming=1&viewType=1,_self}" ;  //并没有走
    ...
    %>
		...
		<script type="text/javascript">
			...
			function onloadtree()
			{
				var ajax=ajaxinit();
       			...
       			ajax.send("loadtree=true&upcoming=1");
			...
			function onloadtreeCount(obj,menus,options,level,customparams){
				jQuery(obj).leftNumMenu("/workflow/request/RequestViewAjaxCount.jsp?<%=request.getQueryString() %>&upcoming=1","update");
			}
			
			function getajaxcounturl() {
				return "/workflow/request/RequestViewAjaxCount.jsp?resourceid=<%=Util.null2String(request.getParameter("resourceid")) %>&upcoming=1";
			}
			...
			function leftMenuClickFn(attr,level,numberType){
				...
				url+="&upcoming=1&viewScope=doing&numberType="+numberType+"&flowAll="+flowAll+"&flowNew="+flowNew+"&flowResponse="+flowResponse+"&flowOut="+flowOut+"&flowSup="+flowSup;
			...
			function WfSearchAll(){
				...
				/*清除右侧查询条件并重新查询*/
				document.getElementById("myFrame").src ="/workflow/search/wfTabFrame.jsp?offical=<%=offical %>&officalType=<%=officalType %>&method=all&upcoming=1&viewScope=doing&complete=0&wftypes=<%=wftypes %>";
			}
		</script>
		...
		<table cellspacing="0" cellpadding="0" class="flowsTable"  >
			<tr>
				...
				<td rowspan="2">
						<iframe id="myFrame" name="myFrame"  src="/workflow/search/wfTabFrame.jsp?offical=<%=offical %>&officalType=<%=officalType %>&method=all&upcoming=1&viewScope=doing&complete=0&wftypes=<%=wftypes %>"
