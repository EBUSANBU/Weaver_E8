...
<%
//,weaver.workflow.request.todo.RequestUtil
	User user = HrmUserVarify.getUser (request , response) ;
    boolean isopenos = false;//RequestUtil.isOpenOtherSystemToDo();//是否开启统一待办
...
<%
	...
	System.out.println("RequestViewAjax==>upcoming:" + upcoming);
	//System.out.println("upcoming: "+ upcoming +" loadtree:" + loadtree);
	if(loadtree.equals("true")){
		...
		if (upcoming.equals("0")){  //待办不含抄送
			...
			sqlsb.append("  from workflow_currentoperator ");
			sqlsb.append("	  ");
			sqlsb.append("    where (    isremark = '8' or isremark = '9')  ");
			sqlsb.append("   and islasttimes = 1 ");
			if("1".equals(belongtoshow)){
				sqlsb.append("	    and userid in (").append(userIDAll);
			}else{
		...
		else {    //防止其他调用此文件的不带upcoming 默认归于此
			sqlsb.append("SELECT wt.id workflowtype,wb.id workflowid FROM workflow_type wt,workflow_base wb,( ");
			sqlsb.append("select workflowtype, workflowid ");
			sqlsb.append("  from workflow_currentoperator ");
			sqlsb.append("	  where ((isremark = '0' and (takisremark is null or takisremark=0)) or isremark = '1' or isremark = '5' or ");
		...
		execute(RecordSet, WorkflowComInfo, fromAdvancedMenu, selectedworkflow, wftypeList, wftypecountList, workflowList, workflowcountList, wftypeworkflowList, wfNodeHahstable, sqlsb);
		//左侧树拼接 json
		demoLeftMenus = "[";
		demoLeftMenus = getString(WorkTypeComInfo, WorkflowComInfo, user, wftypeList, wftypecountList, workflowList, workflowcountList, wftypeworkflowList, wfNodeHahstable, newremarkwfcount0Map, newremarkwfcount1Map, wfovertimecountMap, wfsupedcountMap, wftypes, demoLeftMenus);
		//查询异构系统数据
      /*  if(isopenos && !upcoming.equals("1")) {
			demoLeftMenus = getString(rs, RecordSet, user, demoLeftMenus);
		}*/
		demoLeftMenus += "]";
		out.clear();
...
%>
<%!
	private void execute(RecordSet recordSet, WorkflowComInfo workflowComInfo, int fromAdvancedMenu, String selectedworkflow, ArrayList wftypeList, ArrayList wftypecountList, ArrayList workflowList, ArrayList workflowcountList, ArrayList wftypeworkflowList, Hashtable wfNodeHahstable, StringBuffer sqlsb) {
	...
	private String getString(WorkTypeComInfo workTypeComInfo, WorkflowComInfo workflowComInfo, User user, ArrayList wftypeList, ArrayList wftypecountList, ArrayList workflowList, ArrayList workflowcountList, ArrayList wftypeworkflowList, Hashtable wfNodeHahstable, Map newremarkwfcount0Map, Map newremarkwfcount1Map, Map wfovertimecountMap, Map wfsupedcountMap, String wftypes, String demoLeftMenus) {
		String typeid;
		String typecount;
		String typename;
		String workflowid;
...
//					System.out.println("====================================************=newremarkwfcount1=" + newremarkwfcount1+ ", " + Util.toScreen(newremarkwfcount1, user.getLanguage()));
		...
		return demoLeftMenus;
	}
/*
	private String getString(RecordSet rs, RecordSet recordSet, User user, String demoLeftMenus) {
