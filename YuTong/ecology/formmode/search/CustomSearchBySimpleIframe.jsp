...
<script language="javascript" src="/formmode/js/customSearchOperate_wev8.js?v=3"></script>
<script language=javascript src="/weaversj/x/yt/js/gotoWorkflow.js"></script>
<style>
.ui-multiselect-menu{
	z-index:9999999;
}
	...
	<table id="topTitle" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td class="rightSearchSpan" style="text-align:right;">
				...
				<%if(customid.equals("3563")){%>
				<input style='overflow:hidden;white-space:nowrap;text-overflow:ellipsis;max-width:300px;color:#FFFFFF!important;' class='e8_btn_top_first' title='发起嫁接流程' value='发起嫁接流程' type='button' onclick='clickAndSend(<%=formID%>)' _disabled='true'>
				<%}%>
...
while (RecordSet.next()) {
	...
	if(RecordSet.getString("id").equals("-1")){
		...
		tableString+="				<col width=\""+temocolwidth+"%\" text=\""+Util.toHtmlForSplitPage(SystemEnv.getHtmlLabelName(722,user.getLanguage()))+"\" column=\"modedatacreatedate\" "+orderkey+"  otherpara=\""+para3+"\" transmethod=\"weaver.formmode.search.FormModeTransMethod.getSearchResultCreateTime\" />";
		...
		tableString+="				<col width=\""+temocolwidth+"%\" text=\""+Util.toHtmlForSplitPage(SystemEnv.getHtmlLabelName(81287,user.getLanguage()))+"\" column=\"id\" "+orderkey+" otherpara=\"column:dataid\" transmethod=\"weaver.formmode.search.FormModeTransMethod.getDataId\" />";
	}else{
		String name = RecordSet.getString("name");
		String label = RecordSet.getString("label");
		String htmltype = RecordSet.getString("httype");
		...
		tableString+="			    <col "+editPlugin+" width=\""+temocolwidth+"%\" text=\""+Util.toHtmlForSplitPage(label)+"\"  column=\""+fieldAlias+"\"  otherpara=\""+para3+"\" "+orderkey+" "+transmethod+" />";
	}
}
...
function getAllExcelOut(){ 
	var tmpurl='/weaver/weaver.common.util.taglib.CreateExcelServer?showOrder=all&iscustomsearch=1&comefrom=frommode&modeCustomid=<%=customid %>';
