...
//============================================删除数据====================================
//转移到search_wev8.js的Del方法中处理 qc390129
...
else if(htmltype.equals("3") &&( type.equals("2") || type.equals("19") || type.equals("290")) ){    //日期
	int tmpopt=2;
    int tmpopt1=4;
    String classStr = "";
	if(type.equals("2")){ //日期
        ...
        <span name="datetype_<%=id%>_opt_span" id="datetype_<%=id%>_opt_span" style="<%=datetype_opt_span_display%>">
            ...
            <%if(issimple){%><%=SystemEnv.getHtmlLabelName(348,user.getLanguage())%><%}%><!-- 从 -->
            <button type=button  class="<%=classStr %>" onclick="onFlownoShowDateTime(con<%=id%>_valuespan,con<%=id%>_value,con<%=id%>_value1)" ></button>
            <input type=hidden name="con<%=id%>_value" id="con<%=id%>_value" value="<%=tmpvalue%>">
            <span name="con<%=id%>_valuespan" id="con<%=id%>_valuespan"><%=tmpvalue%></span>
            <select class=inputstyle  name="con<%=id%>_opt1" style="<%=display%>width:90"  >
            <option value="1" <%if(tmpopt1==1){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(15508,user.getLanguage())%></option><!-- 大于 -->
            ...
            <%if(issimple){%><%=SystemEnv.getHtmlLabelName(349,user.getLanguage())%><%}%><!-- 到 -->
            <button type=button  class="<%=classStr %>" onclick="onFlownoShowDateTime(con<%=id%>_value1span,con<%=id%>_value1,con<%=id%>_value)" ></button>
            <input type=hidden name="con<%=id%>_value1" id="con<%=id%>_value1" value="<%=tmpvalue1%>">
            <span name="con<%=id%>_value1span" id="con<%=id%>_value1span"><%=tmpvalue1%></span>
        </span>
    <%
...
if(viewtype!=3&&!norightlist.equals("1")&&!isVirtualForm&&!rightsql.equals("")){//不是监控、无权限列表、不是虚拟表单
...
var index=0;	
function FixedPage(){   
	var t = jQuery("#_xTable").children().children("div.xTable_info:not(.xTable_infoTop)").css("right");
	jQuery("#_xTable").children().children("div.xTable_info:not(.xTable_infoTop)").attr("style","position: fixed; right:0px;");
	index++;
	if(t==undefined&&index<30){
		setTimeout(function(){
			FixedPage();				
		},500);
	}	
...
function Add(){
    ...
    var refurl=decodeURI( window.document.referrer);//地址中带有p_开头的参数要带到新建页面中去
    var reg = /[&?]p_([^&]+)?=([^&]+)?/g; 
